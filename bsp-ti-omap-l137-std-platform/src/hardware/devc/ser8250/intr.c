...
process_lsr(DEV_8250 *dev, unsigned char lsr) {
	...
	if((lsr & (LSR_BI|LSR_OE|LSR_FE|LSR_PE)) == 0)
	{
		...
		write_8250(dev->port[REG_IE], IE_SET_ALL);
#endif
		return(0);
	}
	...
	if(lsr & LSR_BI)
	{
		...
		write_8250(dev->port[REG_IE], (IE_SET_ALL & ~IE_LS));
#endif		
	}
	else
	{
...
ser_intr(void *area, int id) {
	...
	do {
		...
		for(dev = list->device; dev != NULL; dev = dev->next) {
             ...
             */
            while((lsr = read_8250(port[REG_LS])) & LSR_RXRDY)
            {
				c = read_8250(port[REG_RX]);
			...
			case II_TX:		// Transmit buffer empty
					...
					// from TL16C752B dual uart
					 while(!(read_8250(port[REG_LS]) & LSR_TXRDY));
#endif					 
