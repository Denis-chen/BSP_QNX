...
init_system_private() {
	...
	//
	optind = 0;
	while((opt = getopt(_argc, _argv, "AF:M:r:j:")) != -1) {
		char		*p;
		unsigned	type;
		paddr_t		start;
		size_t		size;
	...
	if(reserved_size > 0) {
		...
		reserved = find_top_ram_aligned(reserved_size, reserved_align ? reserved_align : lsp.system_private.p->pagesize);
		if(reserved == NULL_PADDR) {
			crash("Could not reserve 0x%l bytes of memory.\n", reserved_size);
		}
		alloc_ram(reserved, reserved_size, reserved_align ? reserved_align:lsp.system_private.p->pagesize );
...
__SRCVERSION("init_system_private.c $Rev: 392387 $");
