...
cpu_alloc_syspage_memory(paddr32_t *cpupagep, paddr32_t *syspagep, unsigned spsize)
{
	...
	/*
	 * WARNING: ARMs are uniprocessor only, so we put the (single) cpupage
	 *			entry at the end of the syspage. On a multiprocessor system
	 *			we would need to allocate a page for each cpupage and a page
	 *			for the syspage, to allow mapping each processor's cpupage
	...
	if (sp->num_cpu == 1) {
		 ...
		 *        The SMP memmgr is currently still work in progress so
		 *        this may have to be tweaked later.
		 */
		...
		/*
		 * FIXME: map the page directory pages so the memmgr can keep all
		 *        the cpus' page table mappings consistent.
		 *        The SMP memmgr is currently still work in progress so
		 *        this may have to be tweaked later.
		 */
		sp->un.arm.L2_vaddr = arm_map(~0, L2_paddr, sp->num_cpu * __PAGESIZE, ARM_PTE_RW);
	}
...
__SRCVERSION("cpu_syspage_memory.c $Rev: 392398 $");
