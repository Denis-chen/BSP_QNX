...
/* The SPI1 base address is the only difference between L137 and L138
 * Hence keeping the macro over here instead of the omapl1xx.h */

#define OMAPL137_SPI1_BASE              0x01E12000
#define	USB_1_1_GPIO_BANK	1
#define	USB_1_1_GPIO_PIN	15

/*
 * Add L137 specific devices to the hardware info section of the syspage
*/
...
void hwi_omapl137()
{
	...
	/* add 3 UART's */
	{
		...
		/* The 3 UARTs have a clock of SYSCLK2 */
	...
	/* add 2 SPI's */
	{
		...
		/* The 2 SPIs have a clock of SYSCLK2 */
		struct hwi_inputclk clksrc = {.clk = cpu_freq, .div = 2};
		...
		HWIATTR_SPI_SET_LOCATION(&spi_attr, OMAPL137_SPI1_BASE, OMAPL1xx_SPI0_SIZE, 0, hwi_find_as(OMAPL137_SPI1_BASE, 1));
		hwi_off = hwibus_add_spi(HWI_NULL_OFF, &spi_attr);
		ASSERT(hwi_find_unit(hwi_off) == 1);
		hwitag_set_inputclk(hwi_off, 0, &clksrc);
	...
	/* add 2 I2C's */
	{
		...
		/* The I2C0 has a clock of AUXCLK, I2C1 has the SYSCLK4 */
	...
	/* add the GPIO module */
	{
		...
		/* The GPIO operates with a clock of SYSCLK4 which is 300MHz / 4 = 75MHz */
	...
	/* add the Timers module */
	{
		...
		/* Assign interrupt vectors to the Timer 1 */
	...
	/* add the dma controller (32 channels). The EDMA is clocked by SYSCLK2.
	 * The EDMA controller has 1 channel controller and 2 transfer controllers
	 */
	{
		/* Add EDMA Channel Controller */
		...
		/* Add EDMA Transfer Controller 0 */
		...
		/* Add EDMA Transfer Controller 1 */
	...
	/* Add the MMCSD module. The MMCSD is clocked by SYSCLK2. */
	{
		...
		tag->gpio_info.wp.bank = 2;
		tag->gpio_info.wp.pin = 1;
		tag->gpio_info.ins.bank = 2;
		tag->gpio_info.ins.pin = 2;
		tag->gpio_info.ins_irq = 44;
	}
}
