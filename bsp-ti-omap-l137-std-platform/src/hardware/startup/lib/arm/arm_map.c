...
#define	CPU_L1_TABLE(cpu)	((L1_paddr + ((cpu) * ARM_L1_SIZE)) & ~0x1f)
...
set_prot(paddr_t paddr, int flags)
{
	if (flags == ARM_MAP_SYSPAGE) {
		if (lsp.cpuinfo.p->flags & ARM_CPU_FLAG_V6) {
			/*
...
map_ptbl_cpu(int cpu, paddr_t ptbl, uintptr_t vaddr)
{
	 ...
	 */
	*pte = (ptbl | armv_pte->kpte_rw) & ~armv_pte->mask_nc;
...
__SRCVERSION("arm_map.c $Rev: 392398 $");
