...
#ifdef VARIANT_omapl1xx
	if(dra446->bsopts.check_ins_irq(hba,irq) > 0) {
...
static void dra446_setup_rx_edma(dra446_ext_t *dra446, paddr_t addr, int len)
{
	...
	param->abcnt        = ( (8 * SCALE_FIFO) << 16) | 4;
	param->dst          = addr;
	param->srcdstbidx   = (4 << 16) | 0;
	param->linkbcntrld  = 0xFFFF;
	param->srcdstcidx   = ((32 * SCALE_FIFO) << 16) | 0;
	param->ccnt         = len / 32 / SCALE_FIFO;
...
static void dra446_setup_tx_edma(dra446_ext_t *dra446, paddr_t addr, int len)
{
	...
	param->linkbcntrld  = 0xFFFF;
	param->srcdstcidx   = (0 << 16) | (32 * SCALE_FIFO);
	param->ccnt         = len / 32 / SCALE_FIFO;
...
static int dra446_setup_pio(SIM_HBA *hba, int len, int dir)
{
	...
	if (dir == MMC_DIR_IN) {
		...
		if (len <= (32 * SCALE_FIFO))
			dra446->piomask = MMCIM_EDATDNE | MMC_EDRRDY;
		else
...
static int dra446_pio_done(SIM_HBA *hba, char *buf, int len, int dir)
{
	...
	if (dir == MMC_DIR_IN) {
		while (nbytes) {
			cnt = nbytes <= (32 * SCALE_FIFO) ? nbytes : (32 * SCALE_FIFO);
			while (cnt) {
			...
			if (nbytes > (32 * SCALE_FIFO)) {
				while (!(in32(base + DRA446_MMCST0) & MMCST0_DRRDY))
					nanospin_ns(1000);
			} else if (nbytes > 0) {
				while (!(in32(base + DRA446_MMCST0) & (MMCST0_DATDNE | MMC_EDRRDY)))
					nanospin_ns(1000);
			} 
		}
...
__SRCVERSION("sim_dra446.c $Rev: 140019 $");
