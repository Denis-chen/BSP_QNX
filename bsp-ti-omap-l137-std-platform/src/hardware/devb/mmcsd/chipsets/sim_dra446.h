...
struct bs_opt {
	...
	uintptr_t ins_gpio_vbase;
	uintptr_t wp_gpio_vbase;
	int ins_gpio;
	int wp_gpio;
	int ins_irq;
	int ins_bank;
	int (*check_ins_irq)(void *data, int irq);
};
#undef MMC_EDRRDY
#define MMC_EDRRDY MMCIM_EDRRDY
/*
 * FOR OMAPL1XX SOC MMCFIFOCTL_FIFO_256 ENABLES 512 BIT FIFO &
 * MMCFIFOCTL_FIFO_128 ENABLES 256 BIT FIFO
 * BY DEFAULT 512 BIT FIFO IS BEING USED
 */
#ifdef OMAPL1XX_FIFO_SIZE_512
#undef SCALE_FIFO
#define SCALE_FIFO 2
#else
#undef MMCFIFOCTL_FIFO_256
#define MMCFIFOCTL_FIFO_256 0
#endif	/* OMAPL1XX_FIFO_SIZE_512 */
#endif	/* VARIANT_omapl1xx */
