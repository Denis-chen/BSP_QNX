...
int dm644x_reset(dm644x_dev_t * dm644x) 
{
	emac_regs	*emac  = dm644x->emac;
        emac_ctrl_regs  *emac_ctrl = dm644x->emac_ctrl;
	...
	while (emac_ctrl->SOFTRESET != 0)
		;

        dm644x->emac_ctrl->C0RXEN = 0x00; // disable int in ctrl reg
        dm644x->emac_ctrl->C0TXEN = 0x00; // disable int in ctrl reg


        dm644x->emac_ctrl->C1RXEN = 0x00; // disable int in ctrl reg
        dm644x->emac_ctrl->C1TXEN = 0x00; // disable int in ctrl reg


        dm644x->emac_ctrl->C2RXEN = 0x00; // disable int in ctrl reg
        dm644x->emac_ctrl->C2TXEN = 0x00; // disable int in ctrl reg

        dm644x->emac_ctrl->C0MISCEN = 0x00; // disable misc int in ctrl reg
        dm644x->emac_ctrl->C1MISCEN = 0x00; // disable misc int in ctrl reg
        dm644x->emac_ctrl->C2MISCEN = 0x00; // disable misc int in ctrl reg
...
static int dm644x_init_ring(dm644x_dev_t *dm644x)
{
	...
	/* Clear the TX DMA and RX DMA Head pointers,set it to 0 */
	...
	/* Clear the Network Statistics registers (do this before setting MACCONTROL register) */
...
static int dm644x_init_memory(dm644x_dev_t * dm644x) 
{
	...
	dm644x->emac_base = mmap_device_io(cfg->io_window_size[0], cfg->io_window_base[0]);
   	dm644x->emac      = (emac_regs *)(dm644x->emac_base + (EMAC_CNTRL_REGS_BASE - EMAC_BASE_ADDR));
        dm644x->emac_ctrl = (emac_ctrl_regs *)(dm644x->emac_base + (EMAC_WRAPPER_BASE_ADDR - EMAC_BASE_ADDR));
...
static int dm644x_config(dm644x_dev_t * dm644x) 
{
	...
	strcpy((char *)cfg->device_description, "TI DM644x-OMAPL1XX EMAC");
	...
	/* Using channel 0 only - Do not configure other channels */
	emac->MACINDEX = 0;
         
	/* Setup the MAC address register */
	ptr_mac = (uint8_t *)cfg->current_address;
        
        emac->MACADDRHI = *((uint32_t *)ptr_mac);
	emac->MACADDRLO = *((uint16_t *)(ptr_mac + 4)) | ( EMAC_MACADDRLO_MATCHFILT | EMAC_MACADDRLO_VALID) ;
	
        /* Clear the HASH registers */
	emac->MACHASH1 = 0;
	emac->MACHASH2 = 0;

	/* Set source MAC address register */
	...
	/* Receive broadcast enable, Broadcast frames are copied to the channel selected by RXBROADCH(channel 0 selected) */
...
int dm644x_register_device(dm644x_dev_t * dm644x, io_net_self_t * ion, void *dll_hdl) 
{
	...
	if (dm644x_config(dm644x))
        ...
        // The correct way to init a pulse
        SIGEV_PULSE_INIT (&event, dm644x->coid, cfg->priority, NIC_INTERRUPT_EVENT, 1);
        if (dm644x->noirq) {
		int                 timerid;
		...
		if (timer_settime(timerid, 0, &value, NULL) == -1)
			goto fail4;
               ...
               if (ThreadCtl( _NTO_TCTL_IO, 0 ) == -1)
                ...
                // This is for Receive Interrupt
                if ((dm644x->iid = InterruptAttachEvent(cfg->irq[0], &event, _NTO_INTR_FLAGS_TRK_MSK)) == -1) {
                nic_slogf(_SLOGC_NETWORK, _SLOG_ERROR, "devn-dm644x: Rx InterruptAttachEvent failed");
                goto fail4;
                }

                // This is for Transmit Interrupt
                if ((dm644x->iid = InterruptAttachEvent(cfg->irq[1], &event, _NTO_INTR_FLAGS_TRK_MSK)) == -1){
		nic_slogf(_SLOGC_NETWORK, _SLOG_ERROR, "devn-dm644x: Tx InterruptAttachEvent failed");
		goto fail4;
	        }
        }
	...
	dm644x->emac->RX0HDP = (unsigned int)(ion_mphys((void *)dm644x->rx_desc));
        ...
        // Disable all Tx/ Rx/ Misc Interrupt Channels
        dm644x->emac->TXINTMASKCLEAR = 0xFF;
        dm644x->emac->MACINTMASKCLEAR = 0x3;

	if (!dm644x->noirq) {
		/* Enable interrupts here */
                dm644x->emac_ctrl->C0RXEN = 0x01; // enable int in ctrl reg
		dm644x->emac_ctrl->C0TXEN = 0x01; // enable int in ctrl reg
...
static int dm644x_process_interrupt(dm644x_dev_t * dm644x) 
{
	...
	irqSrc = emac->MACINVECTOR; // shows *all* interrupting sources - reduces register reads
	if (!irqSrc) return 0; // nothing to do - useful optimisation if we're attached to the timer interrupt

	pthread_mutex_lock(&dm644x->mutex);

	if (irqSrc & MACINVECTOR_RX0PEND)
	{
		dm644x_receive(dm644x);
		emac->MACEOIVECTOR = ACK_CORX_INT; // clear interrupt
		if (!dm644x->noirq)
			InterruptUnmask(dm644x->cfg.irq[0], dm644x->iid);
	}

	if (irqSrc & MACINVECTOR_TX0PEND)
	{
		dm644x_transmit(dm644x);
		emac->MACEOIVECTOR = ACK_COTX_INT;// clear interrupt
		if (!dm644x->noirq)
			InterruptUnmask(dm644x->cfg.irq[1], dm644x->iid);
	}
...
/* This is a debug function, can be used to dump the EMAC registers configuration */
void  dump_emac_cfg(dm644x_dev_t * dm644x)
{
  unsigned int *emac_addr;

  printf("Dumping EMAC Control Module Registers(0x01E2 2000 - 0x01E2 2084) : \n");
  emac_addr = (unsigned int *)dm644x->emac_ctrl;
  while(emac_addr <= (((unsigned int *)dm644x->emac_ctrl) + (0x84/4))){
     printf("Addr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\n",
              ((unsigned int)emac_addr - (unsigned int)dm644x->emac_ctrl), *emac_addr,(((unsigned int)emac_addr -(unsigned int)dm644x->emac_ctrl) +4), *(emac_addr+1),
              (((unsigned int)emac_addr -(unsigned int)dm644x->emac_ctrl) +8), *(emac_addr+2),(((unsigned int)emac_addr - (unsigned int)dm644x->emac_ctrl) +12), *(emac_addr+3));
     emac_addr += 4;
  }
  
  printf("\nDumping EMAC Control Registers(0x01E2 3000 - 0x01E2 3FFF) : \n");
  emac_addr = (unsigned int *)dm644x->emac;
   while(emac_addr <= (((unsigned int *)dm644x->emac) + (0x1000/4))){
     printf("Addr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\n",
              ((unsigned int)emac_addr - (unsigned int )dm644x->emac), *emac_addr,(((unsigned int)emac_addr - (unsigned int )dm644x->emac)+4), *(emac_addr+1),
              (((unsigned int)emac_addr - (unsigned int )dm644x->emac)+8), *(emac_addr+2),(((unsigned int)emac_addr - (unsigned int )dm644x->emac)+12), *(emac_addr+3));
     emac_addr += 4;
  }
  
  printf("\nDumping EMAC MDIO Registers(0x01E2 4000 - 0x01E2 408C) : \n");
  emac_addr = (unsigned int *)dm644x->mdio;
  while(emac_addr <= (((unsigned int *) dm644x->mdio) + (0x8C/4))){
     printf("Addr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\tAddr: 0X%04X\tVal : 0X%08X\n",
              ((unsigned int)emac_addr - (unsigned int )dm644x->mdio), *emac_addr,(((unsigned int)emac_addr - (unsigned int )dm644x->mdio)+4), *(emac_addr+1),
              (((unsigned int)emac_addr - (unsigned int )dm644x->mdio)+8), *(emac_addr+2),(((unsigned int)emac_addr - (unsigned int )dm644x->mdio) +12), *(emac_addr+3));
     emac_addr += 4;
  }
  printf("Done..\n");
}
