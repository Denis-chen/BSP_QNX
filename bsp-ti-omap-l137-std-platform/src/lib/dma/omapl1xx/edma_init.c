...
void init_once(edma_t *edma)
{
	...
	for(j = 0; j < edma->cc_nos; j++) {
		...
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_DRAE(0), ~0);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_QRAE(0), ~0);
		/* Disable interrupts for all channels */
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_IECR, ~0UL);
		
		/* disable event for all channels */
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_ECR, ~0UL);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_EECR, ~0UL);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_QEECR, ~0UL);
		
		/* clear up all previous pending interrupts */
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_SECR, ~0UL);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_QSECR, ~0UL);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_ICR, ~0UL);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_EMCR, ~0UL);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_QEMCR, ~0UL);
		out32(edma->cc[j].vbase + OMAPL1xx_EDMA_CCERRCLR, ~0UL);

		/* map all QDMA channels to param, from the last param used for edma */
		for(i = 0; i < edma->cc[j].qdma_chnos; i++)
			out32(edma->cc[j].vbase + OMAPL1xx_EDMA_QCHMAP(i), PAENTRY( edma->cc[j].edma_chnos + i));
	}
}
