...
static inline int __attribute__((always_inline)) omapl1xx_edma_param(int paramno)
{
	return (OMAPL1xx_EDMA_PARAM_BASE + ((paramno) * 32));
}
...
static inline param_t* __attribute__((always_inline)) get_param(channel_t *ch)
{
	...
	return ((void*)ch->cc->vbase + omapl1xx_edma_param(paramno));
}
