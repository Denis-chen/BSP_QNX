...
/* This struct is having configuration for both l138 & l137
 * DMA lib can be used for l138/l137 by 
 * giving suitable options to init (variant=l138/l137)
 */
static edmacc_t edmacc_s[OMAPL1xx_MAX_CC_NO] = {
		{
			.pbase = OMAPL1xx_EDMA0_CC_BASE,
			.edma_chnos = OMAPL1xx_EDMA0_NUM_CHAN,
			.qdma_chnos = OMAPL1xx_QDMA0_NUM_CHAN,
			.irq_no	= 0xc100,
			.size = OMAPL1xx_EDMA_CC_SIZE,
			.tcno = OMAPL1xx_EDMA0_TCNOS
		},
		{
			.pbase = OMAPL1xx_EDMA1_CC_BASE,
			.edma_chnos = OMAPL1xx_EDMA1_NUM_CHAN,
			.qdma_chnos = OMAPL1xx_QDMA1_NUM_CHAN,
			.irq_no	= 0xc120,
			.size = OMAPL1xx_EDMA_CC_SIZE,
			.tcno = OMAPL1xx_EDMA1_TCNOS
		}
};

static edma_t edma = {
	.cc = edmacc_s,
	.cc_nos = OMAPL1xx_CC_NOS
};
...
static char *const init_options[] = {
	[VARIANT]					= "variant",	/* variant=l138/l137 */
	[EDMA0_IRQ]					= "edma0irq",
	[EDMA1_IRQ]					= "edma1irq",
	[EDMA0_BASE]				= "edma0base",
	[EDMA1_BASE]				= "edma1base",
	NULL
};
...
static int parse_init_opt(char *options, edma_t *edmap)
{
	...
	if(options)
		while((opt = getsubopt(&options, init_options, &valuep)) != -1) {
			switch(opt) {
				case VARIANT:
						if(!strcmp(valuep, "l138")) {
							edmap->cc_nos = 2;
							slog("setting to variant %s", valuep);
						} else if(!strcmp(valuep, "l137")) {
							edmap->cc_nos = 1;
							slog("setting to variant %s", valuep);
						}
					break;
				case EDMA0_IRQ:
					...
					break;
				case EDMA1_IRQ:
					edmap->cc[1].irq_no = strtoul(valuep, NULL, 0);
					break;
				case EDMA0_BASE:
					edmap->cc[0].pbase = strtoul(valuep, NULL, 0);
					break;
				case EDMA1_BASE:
					edmap->cc[1].pbase = strtoul(valuep, NULL, 0);
					break;
				default:
...
static void print_edma_info(edma_t edma)
{
	...
	for(i = 0; i < edma.cc_nos; i++) {
			cc = &edma.cc[i];
			slog("Channel Controller%d revision = %x", i, in32(cc->vbase + OMAPL1xx_EDMA_PID));
			slog("Channel Controller%d Configuration = %x", i, in32(cc->vbase + OMAPL1xx_EDMA_CCCFG));
	}
}
...
/* Channel arrangement for L183
 *  	CHANNEL CONTROLLER 0 EDMA CHANNELS => 0 - 31
 *		CHANNEL CONTROLLER 1 EDMA CHANNELS => 32 - 63
 *		CHANNEL CONTROLLER 0 QDMA CHANNELS => 64 - 71
 *		CHANNEL CONTROLLER 1 QDMA CHANNELS => 71 - 79
 * Channel arrangement for L187
 *  	CHANNEL CONTROLLER 0 EDMA CHANNELS => 0 - 31
 *		CHANNEL CONTROLLER 1 EDMA CHANNELS => 32 - 39
 */
...
static int edma_channel_info(unsigned int channel, dma_channel_info_t *channelinfo)
{
	...
	channelinfo->max_xfer_size = (unsigned long)OMAPL1xx_MAX_XFER_SIZ; /* ACNT * BCNT */
	channelinfo->max_src_segments = OMAPL1xx_MAX_SRC_SEG; /* BCNT */
	channelinfo->max_dst_segments = OMAPL1xx_MAX_DST_SEG; /* BCNT */
	/* no:of dst & src fragments is not constant, depends upon the no:of free params */
	channelinfo->max_src_fragments = OMAPL1xx_MAX_SRC_FRAG;
	channelinfo->max_dst_fragments = OMAPL1xx_MAX_DST_FRAG;
	channelinfo->xfer_unit_sizes = OMAPL1xx_XFER_UNIT_SIZ; /* DBS */;
	channelinfo->mem_upper_limit = OMAPL1xx_MEM_UPPER_LIM;
	channelinfo->mem_lower_limit = OMAPL1xx_MEM_LOWER_LIM;
	channelinfo->mem_nocross_boundary = OMAPL1xx_MEM_NOCROSS;
...
static int edma_driver_info(dma_driver_info_t *driverinfo)
{
	...
	driverinfo->lib_rev = OMAPL1xx_LIB_REVISION;
	driverinfo->description = OMAPL1xx_DMA_NAME;
	driverinfo->num_channels = OMAPL1xx_MAX_NUM_CHANNELS;
	driverinfo->max_priority = OMAPL1xx_MAXPRIO;	
	return 0;
}
...
/*
 * Channel number can either be as given below
 *  	CHANNEL CONTROLLER 0 EDMA CHANNELS => 0 - 31
 *		CHANNEL CONTROLLER 1 EDMA CHANNELS => 32 - 63
 *		CHANNEL CONTROLLER 0 QDMA CHANNELS => 64 - 71
 *		CHANNEL CONTROLLER 1 QDMA CHANNELS => 71 - 79
			    ...
			    int priority, dma_attach_flags flags)
{
	...
	*channel = ch->chno = req.start;
	ch->prio = priority;
	ch_arr[ch->chno] = ch;
	uint32_t tmp = in32(cc->vbase + OMAPL1xx_EDMA_DRAE(0));
	slog("attached channel %d", ch->chno);

	/* setup DRAE0 for the channel */
	if(is_edma(ch)) {
		tmp = in32(cc->vbase + OMAPL1xx_EDMA_DRAE(0));
		out32(cc->vbase + OMAPL1xx_EDMA_DRAE(0), tmp | setbit(ch->chno));
	} else if(is_qdma(ch)) {
		tmp = in32(cc->vbase + OMAPL1xx_EDMA_QRAE(0));
		out32(cc->vbase +  OMAPL1xx_EDMA_QRAE(0), tmp | setbit(ch->chno));
	}
	...
	if((priority || !(flags & DMA_ATTACH_PRIORITY_HIGHEST)) && cc->tcno > 1) {
		...
		if(is_edma(ch)) {
			tmp = in32(cc->vbase +  OMAPL1xx_EDMA_DMAQNUM(ch->chno >> 3));
			out32(cc->vbase +  OMAPL1xx_EDMA_DMAQNUM(ch->chno >> 3), tmp | setbit((ch->chno & 7) << 2));
		} else if(is_qdma(ch)) {
			tmp = in32(cc->vbase + OMAPL1xx_EDMA_QDMAQNUM);
			out32(cc->vbase +  OMAPL1xx_EDMA_QDMAQNUM, tmp | setbit((ch->chno << 2)));
		}
	}
...
static int cleanup_params(channel_t *ch)
{
	...
	uint32_t tmp;
	param_t *param = (param_t *) (cc->vbase + (is_edma(ch) ? omapl1xx_edma_param(ch->chno) : omapl1xx_edma_param(ch->chno + cc->edma_chnos)));
...
static int	edma_setup_xfer(void *handle, const dma_transfer_t *tinfo)
{
	...
	if( (tinfo->src_flags & DMA_ADDR_FLAG_SCATTER_GATHER || tinfo->dst_flags & DMA_ADDR_FLAG_SCATTER_GATHER) &&
		(tinfo->src_fragments > OMAPL1xx_MAX_SRC_FRAG || tinfo->dst_fragments > OMAPL1xx_MAX_DST_FRAG) ) {
	...
	paramno = req[0].start;
	param = (param_t*) ((void*)cc->vbase + omapl1xx_edma_param(paramno));
	...
	if(is_edma(ch))
		...
		tmp = in32(cc->vbase + OMAPL1xx_EDMA_QCHMAP(ch->chno));	/* all edma are mapped to 1 ipr bit */
		out32(cc->vbase + OMAPL1xx_EDMA_QCHMAP(ch->chno), 
							tmp | OMAPL1xx_EDMA_QCHMAP_TRWORD_CCNT);	/* setup the trigger zone */
	}
	...
	if(tinfo->src_flags & DMA_ADDR_FLAG_SCATTER_GATHER || tinfo->dst_flags & DMA_ADDR_FLAG_SCATTER_GATHER) {
		...
		req[1].end = OMAPL1xx_MAX_PARAM_NO;
		req[1].name = chparam;
		...
		ch->sg_param = (param_t*) ((void*)cc->vbase + omapl1xx_edma_param(req[1].start));
		...
		bcnt = tmp / acnt;
		param->a_b_cnt = PARAM_ACNT(acnt) | PARAM_BCNT(bcnt);
		param->link_bcntreload = req[1].length ? PARAM_LINK(omapl1xx_edma_param(req[1].start)) : PARAM_LINK_NULL;
		...
		for(i = 0; i < req[1].length && req[1].length; i++ ) {
			...
			tmp_param->link_bcntreload = (i == req[1].length - 1) ? PARAM_LINK_NULL : PARAM_LINK(omapl1xx_edma_param(req[1].start + i + 1));
		}
	}
	...
	if(tinfo->mode_flags & DMA_MODE_FLAG_REPEAT) {
		...
		req[0].end = OMAPL1xx_MAX_PARAM_NO;
		req[0].length = 1;
		req[0].name = chparam;
		req[0].flags = RSRCDBMGR_FLAG_NAME | RSRCDBMGR_FLAG_RANGE;
		...
		ch->sg_param->link_bcntreload |= omapl1xx_edma_param(req[1].start);	/* link to itsef */
	}
	if(edma_init_irq(ch))
		goto fail2;
...
static void edma_channel_release(void *handle)
{
	...
	edma_fini_irq(ch);
	if(is_edma(ch))	{
		//out32(cc->vbase + OMAPL1xx_EDMA_IECR, setbit(ch->chno));				/* disable interrupt */
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_ECR, setbit(ch->chno));					/* disable event */
		out32(cc->vbase + OMAPL1xx_EDMA_EECR, setbit(ch->chno));					/* disable event */
		tmp = in32(cc->vbase +  OMAPL1xx_EDMA_DMAQNUM(ch->chno >> 3));
		out32(cc->vbase +  OMAPL1xx_EDMA_DMAQNUM(ch->chno >> 3), tmp & ~setbit((ch->chno & 7) << 2));	/* reset to use TC0 (high prio) */
		tmp = in32(cc->vbase + OMAPL1xx_EDMA_DRAE(0));
		out32(cc->vbase + OMAPL1xx_EDMA_DRAE(0), tmp & ~setbit(ch->chno));		/* disable access to shadow region 0 */
	} else if(is_qdma(ch)) {
		/* we dont disable qdma interrupt, for that is shared between all qdma channels */
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_QEECR, setbit(ch->chno));				/* disable event */
		tmp = in32(cc->vbase + OMAPL1xx_EDMA_QDMAQNUM);
		out32(cc->vbase +  OMAPL1xx_EDMA_DMAQNUM(ch->chno >> 3), tmp & ~setbit(ch->chno<< 2));	/* reset to use TC0 (high prio) */
		tmp = in32(cc->vbase + OMAPL1xx_EDMA_QRAE(0));
		out32(cc->vbase +  OMAPL1xx_EDMA_QRAE(0), tmp & ~setbit(ch->chno));		/* disable access to shadow region 0 */
		chno += cc->edma_chnos;
	}
...
static int edma_xfer_start(void *handle)
{
	...
	if(is_edma(ch)) {
		...
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_EECR, setbit(ch->chno));
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_IECR, setbit(ch->chno));

		/* clear up all pending events from previous events */
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_ECR, setbit(ch->chno));
		out32(cc->vbase + 0x1000 + OMAPL1xx_EDMA_SECR, setbit(ch->chno));
		out32(cc->vbase + OMAPL1xx_EDMA_EMCR, setbit(ch->chno));
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_ICR, setbit(ch->chno));
		
		/* enable interrupts */
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_IESR, setbit(ch->chno));
		/* events must be enbled always */
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_EESR, setbit(ch->chno));
		...
		if(!(ch->flags & DMA_ATTACH_CASCADE)) {
			...
			out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_ESR, setbit(ch->chno));
		}
	} else if(is_qdma(ch)) {
		/* disabling event & interrupt before cleaning up previously enabled interrupts */
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_QEECR, setbit(ch->chno));
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_IECR, qdma_ipr_no);

		/* clear up all pending events from previous events */
		out32(cc->vbase + OMAPL1xx_EDMA_QEMCR, setbit(ch->chno));
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_QSECR, setbit(ch->chno));
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_ICR, qdma_ipr_no);
		
		/* enable interrupts & events */
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_IESR, qdma_ipr_no);
		out32(cc->vbase + OMAPL1xx_EDMA_REGION0 + OMAPL1xx_EDMA_QEESR, setbit(ch->chno));
		
		/* trigger the qdma channel txr by writing to the trigger field
		 * we set up a constant trigger field here ,ie ccnt
		 */
		param = (param_t *) (cc->vbase + omapl1xx_edma_param(ch->chno + cc->edma_chnos));
		param->ccnt = 1;
	}
