...
#include <dsplink160/drv_api.h>
#include <dsplink160/msgqdefs.h>
...
#if 0
#define CMD_PROC_CLOSEFILEDESC             (PROC_BASE_CMD + 15)
#endif
 ...
 */
	typedef struct CMD_Args_tag {
		...
		union {
			struct {
				Int32	cfg_len; //length of the GPP config passing through client
			...
			struct {
				char *		notify_fd;
				Uint32		size;
			} notifysyncCreateArgs;
	
			struct {
				char *		notify_fd;
			} notifysyncDeleteArgs;
